...
deploy:
	...
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && echo "API_MAILER_HOST=${API_MAILER_HOST}" >> .env'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && echo "API_MAILER_PORT=${API_MAILER_PORT}" >> .env'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && echo "API_MAILER_USER=${API_MAILER_USER}" >> .env'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && echo "API_MAILER_PASSWORD=${API_MAILER_PASSWORD}" >> .env'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && echo "API_MAILER_FROM_EMAIL=${API_MAILER_FROM_EMAIL}" >> .env'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && echo "SENTRY_DSN=${SENTRY_DSN}" >> .env'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && docker-compose -f docker-compose-production.yml pull'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && docker-compose -f docker-compose-production.yml up --build -d api-postgres api-php-cli'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && docker-compose -f docker-compose-production.yml run api-php-cli wait-for-it api-postgres:5432 -t 60'
	ssh ${HOST} -p ${PORT} 'cd control-desk_${BUILD_NUMBER} && docker-compose -f docker-compose-production.yml run api-php-cli php bin/app.php migrations:migrate --no-interaction'
